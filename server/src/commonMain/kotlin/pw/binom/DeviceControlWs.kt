...
class DeviceControlWs(
    ...
    private val sender = ProcessingChannel<ServerMessage> { input ->
        logger.info("Sending to device... $input")
        try {
            ...
            logger.warn(text = "Can't send message to device", exception = e)
        }
    }

    override suspend fun income(message: ServerMessage) {
        logger.info("push $message to device")
        sender.push(message)
    }
